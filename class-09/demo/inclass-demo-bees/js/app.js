...
// event handler first step:  window into DOM
let hiveForm = document.getElementById('hive-form');
// simulate hours arrray with a yield array
let yields = ['yield one', 'yield two', 'yield three'];
...
function calculateTotals() {
  for (let i = 0; i < yields.length; i++) {
    ...
    for (let j = 0; j < allHives.length; j++) {
      columnTotal += allHives[j].honeyYieldArray[i];
    }
    totalsArray.push(columnTotal);
  }
}
...
function handleSubmit(event) {
  ...
  let hiveName = event.target.name.value;
  let yieldOne = +event.target.yieldone.value;
  let yieldTwo = +event.target.yieldtwo.value;
  let yieldThree = parseInt(event.target.yieldthree.value);
  let imgSrc = event.target.source.value;

  let honeyYield = [yieldOne, yieldTwo, yieldThree];
  console.log(hiveName);

  // let newHive = new Hive(hiveName, honeyYield, imgSrc);
  // newHive.render();

  new Hive(hiveName, honeyYield, imgSrc);
...
hiveForm.addEventListener('submit', handleSubmit);
